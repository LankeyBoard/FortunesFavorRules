...
const isHrefInPath = (partialPath: partialPath, navHref: string): boolean => {
  ...
  if (splitHref.length > 1 && partialPath.hash) {
    if (
      partialPath.pathname.includes(splitHref[0]) &&
      partialPath.hash.includes(splitHref[1])
    )
      return true;
    else return false;
  } else if (
...
export const NavElem = ({
  ...
  const href = {
    ...
    href: `${path.origin}${path.pathname}${path.search.toString()}${path.hash}`,
  };
...
function findClosestHref(navIdMap: {
  [key: string]: { id: string; loc: number };
}): partialPath | undefined {
  //update id locations
  for (const [key, elem] of Object.entries(navIdMap)) {
    const element = document.getElementById(elem.id);
    if (element) navIdMap[key].loc = element.getBoundingClientRect().top;
  }
...
const NavMenu = ({ navMap }: { navMap: nav[] }) => {
  ...
  const [path, setPath] = useState<Location | undefined>(
  ...
  const handleScroll = () => {
    ...
    if (closestHref !== path && closestHref) {
      ...
      const newPath: Location = {
        ...path,
        pathname:
          closestHref.pathname[0] === "/"
            ? closestHref.pathname
            : "/" + closestHref.pathname,
        hash: closestHref.hash || "",
        href:
          path.origin + closestHref.pathname + path.search + closestHref.hash ||
          "",
      };
      setPath(newPath);
    }
  ...
  useEffect(() => {
    ...
    const id = setTimeout(
      (path) => {
        ...
        router.replace(
          `${path.pathname}${path.search.toString()}${path.hash}`,
          { scroll: false },
        );
      },
      500,
