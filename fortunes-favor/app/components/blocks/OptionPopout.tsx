...
const OptionPopout = ({child, showPopout, isSelected=false, setSelectedSlug, large=false}: popoutProps) => {
    if(!child)
        return;
    const [selected, setSelected] = useState(isSelected);
    const popoutStyle = "h-min max-h-full bg-slate-700 p-4 items-start border-2 relative overflow-y-auto cursor-pointer ".concat(!selected? "border-amber-700 " : "border-emerald-600 ").concat(large?"w-3/4":"w-1/2");
            ...
            <div className="my-40 flex justify-center w-full">
                ...
                onClick={(e)=> {e.stopPropagation(); 
                    ...
                    <div className="relative bottom-0 right-0 m-2 clear-both float-right cursor-pointer">
                        <button 
                            className={!selected? "p-3 bg-emerald-600 hover:bg-emerald-500 mx-3 rounded-md": "p-3 bg-amber-700 mx-3 rounded-md"}
                            onClick={(e)=> {e.stopPropagation(); 
