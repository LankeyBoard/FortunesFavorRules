...
const InputArea = ({name, isRequired=false, defaultValue, rows=2}: inputAreadProps) => {
    const inputClass = "block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none overflow-hidden dark:text-white dark:border-gray-600 dark:focus:border-amber-500 focus:outline-none focus:ring-0 focus:border-amber-600 peer";
    ...
    return(
        <div className="relative z-0 max-w-lg mb-5 group">
            <textarea name={name} id={name} className={inputClass} required={isRequired} defaultValue={defaultValue} rows={rows} onInput={(e) => {e.currentTarget.style.height = "";e.currentTarget.style.height = e.currentTarget.scrollHeight + "px"}}/>
